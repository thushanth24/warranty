...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Expose warranty schema as JSON Schema
  app.get("/api/schema/warranty", (req, res) => {
    const jsonSchema = zodToJsonSchema(insertWarrantySchema, "Warranty");
    res.json(jsonSchema);
  });
  ...
  app.post("/api/auth/verify-otp", async (req, res) => {
    try {
      ...
      console.log("[DEBUG] Returning user object after OTP verification:", user);
  ...
  app.post("/api/subscriptions/:userId", async (req, res) => {
    try {
      ...
      // Convert nextRenewalDate from string to Date if needed
      if (typeof req.body.nextRenewalDate === "string") {
        req.body.nextRenewalDate = new Date(req.body.nextRenewalDate);
      }
  ...
  app.put("/api/subscriptions/:userId/:id", async (req, res) => {
    try {
      ...
      // Convert nextRenewalDate from string to Date if needed
      if (typeof updates.nextRenewalDate === "string") {
  ...
  app.put("/api/warranties/:userId/:id", async (req, res) => {
    try {
      ...
      console.log('Incoming warranty update payload:', updates);
      // Preprocess date fields if present
      ...
      const err: any = error;
      console.error("Error updating warranty:", err && err.stack ? err.stack : err);
