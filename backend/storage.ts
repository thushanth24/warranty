...
export class MemStorage implements IStorage {
  ...
  async createWarranty(warranty: InsertWarranty & { userId: number }): Promise<Warranty> {
    ...
    const newWarranty: Warranty = {
      ...
      vendorEmail: warranty.vendorEmail ?? null,
      vendorPhone: warranty.vendorPhone ?? null,
      vendorWebsite: warranty.vendorWebsite ?? null,
      vendorAddress: warranty.vendorAddress ?? null,
      receiptPhotos: warranty.receiptPhotos ?? [],
      warrantyDocuments: warranty.warrantyDocuments ?? [],
      isTransferred: warranty.isTransferred ?? false,
      transferredTo: warranty.transferredTo ?? null,
      transferDate: warranty.transferDate ?? null,
      transferNotes: warranty.transferNotes ?? null,
  ...
  async createWarrantyClaim(insertClaim: InsertWarrantyClaim & { userId: number }): Promise<WarrantyClaim> {
    ...
    const claim: WarrantyClaim = {
      ...insertClaim,
      id,
      warrantyId: insertClaim.warrantyId,
      userId: insertClaim.userId,
      claimNumber: insertClaim.claimNumber ?? null,
      issueDescription: insertClaim.issueDescription ?? '',
      claimDate: insertClaim.claimDate ?? new Date(),
      status: insertClaim.status ?? null,
