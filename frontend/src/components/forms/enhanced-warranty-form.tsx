...
import { insertWarrantySchema as baseWarrantySchema } from "../../../shared/schema";

// Extend base schema for frontend to expect string for date fields
const enhancedWarrantySchema = baseWarrantySchema.extend({
...
export default function EnhancedWarrantyForm({ warranty, onSuccess, onCancel }: EnhancedWarrantyFormProps) {
  // Top-level debug button for event system check
  if (typeof window !== 'undefined') {
    window['__ewf_render_count'] = (window['__ewf_render_count'] || 0) + 1;
    console.log('EnhancedWarrantyForm render count:', window['__ewf_render_count']);
  }
  console.log("EnhancedWarrantyForm rendered", { warranty });

  // All hooks and logic must be at the top of the component, not inside return
  const { user } = useAuth();
  ...
  const form = useForm<EnhancedWarrantyForm>({
    ...
    defaultValues: {
      ...
      expirationDate: warranty?.expirationDate ? new Date(warranty.expirationDate).toISOString().split('T')[0] : '',
  ...
  const createWarrantyMutation = useMutation({
    mutationFn: async (data: EnhancedWarrantyForm) => {
      ...
      console.log("Submitting enhanced warranty - raw form data:", data);
      console.log("Current user:", user);
      console.log("Warranty prop:", warranty);
      console.log("apiRequest function:", apiRequest);
      console.log("purchaseDate type before conversion:", typeof data.purchaseDate, data.purchaseDate);
      console.log("expirationDate type before conversion:", typeof data.expirationDate, data.expirationDate);
      ...
      console.log("purchaseDate type after conversion:", typeof purchaseDate, purchaseDate);
      console.log("expirationDate type after conversion:", typeof expirationDate, expirationDate);
      const warrantyData = {
        ...data,
        userId: user?.id,
        purchaseDate,
      ...
      if (warranty) {
        return await apiRequest("PUT", `/api/warranties/${user?.id}/${warranty.id}`, {
          body: JSON.stringify(warrantyData),
        });
      } else {
        return await apiRequest("POST", `/api/warranties/${user?.id}`, {
          body: JSON.stringify(warrantyData),
        });
      }
    },
    ...
    onError: async (error) => {
      console.error("EnhancedWarrantyForm mutation error", error);
      let description = "Failed to save warranty. Please try again.";
    ...
    onSettled: (data, error) => {
      console.log("EnhancedWarrantyForm mutation settled. Data:", data, "Error:", error);
    },
  });
  ...
  return (
    <>
      <button style={{position: 'fixed', top: 0, left: 0, zIndex: 9999, background: 'yellow'}} onClick={() => console.log('Top debug button clicked')}>Top Debug Button</button>
      {/* Main component render follows */}
      <div className="max-w-4xl mx-auto space-y-6">
        <Card>
          ...
          <CardContent>
            <Form {...form}>
              <form onSubmit={e => { console.log('form onSubmit event fired'); form.handleSubmit(onSubmit)(e); }} className="space-y-6">
                {/* Product Information */}
                {/* ...rest of form fields/components... */}
              </form>
            </Form>
          </CardContent>
        </Card>
  ...
  const onSubmit = (data: EnhancedWarrantyForm) => {
    console.log('[EnhancedWarrantyForm] onSubmit called with data:', data);
